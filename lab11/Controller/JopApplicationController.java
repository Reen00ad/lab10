...
public class JopApplicationController {
    ...
    public ResponseEntity addJopapplication(@RequestBody @Valid JopApplication jopApplication, Errors errors) {
        ...
        Integer isadded=jopApplicationService.addJopApplication(jopApplication);
        if(isadded==3) {
            return ResponseEntity.status(400).body(new ApiResponce("user id not found"));

        }if(isadded==2) {
            return ResponseEntity.status(400).body(new ApiResponce("jop post id not found"));
        }
